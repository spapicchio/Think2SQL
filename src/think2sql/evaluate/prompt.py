...
def get_prompt_process_messages_bird(user_prompt_name,
                                     system_prompt,
                                     prompt_folder,
                                     add_sample_rows_strategy: Optional[Literal["append", "inline"]],
                                     relative_base_path: str) -> Callable:
    def prompt_process_messages_bird(line, task_name) -> Doc:
    ...
    return prompt_process_messages_bird
...
def get_prompt(evaluate_args: EvaluateArgs):
    if evaluate_args.task_name == 'bird_dev':
        ...
        return get_prompt_process_messages_bird(
