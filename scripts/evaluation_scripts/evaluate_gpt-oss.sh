#!/bin/bash
#SBATCH --job-name=eval-gpt-oss-6
...
# By default the reasoning effort is set to `medium`  https://huggingface.co/openai/gpt-oss-20b/blob/main/chat_template.jinja
#https://cookbook.openai.com/articles/gpt-oss/run-transformers
MODEL_NAME='openai/gpt-oss-20b'  # ~48 GB
...
# defined in utils.sh
launch_vllm "$CUDA_VISIBLE_DEVICES" "$MODEL_NAME" "$VLLM_SERVER_HOST" "$VLLM_SERVER_PORT" "$DATA_PARALLEL_SIZE" "$TENSOR_PARALLEL_SIZE"
...
# run_suite
 ...
 run with best parameters from HF https://huggingface.co/openai/gpt-oss-120b/discussions/21
TEMP=1.0
TOP_P=1.0
TOP_K=0
REP_PENALTY=1.00
